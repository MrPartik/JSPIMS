...
if(!isset($_POST['prog'])){
	...
	$query ="SELECT *, IF (A.exam_taken = 0,'No', 'Yes') AS userstatus FROM APPLICANT AS A INNER JOIN ACADYR AS B ON A.ACADYR_FK = B.ACADYR INNER JOIN ACC AS C ON A.ACC_FK = C.ACCID INNER JOIN SCHED AS D ON A.SCHED_FK = D.SCHED_ID INNER JOIN ROOM AS E ON D.ROOM_FK = E.ROOM_ID where D.SCHED_ID = '$prog' ORDER BY A.APP_ID DESC";
}
$query2="SELECT * FROM SCHED AS A INNER JOIN ACADYR AS B ON A.ACADYR_FK = B.ACADYR where A.SCHED_ID = '$prog' AND A.SCHED_STAT = 0";
$result = mysqli_query($connect, $query);
$results = mysqli_query($connect, $query2);
$numrow =mysqli_num_rows($result);
while($rows = mysqli_fetch_array($results))
{
			$program = $rows['SCHED_TIME']. '-'. $rows['SCHED_DATE'].'/ A.Y. '.$rows['ACADYR_NAME'];
}
