...
$prog = $_POST["prog"];
if(!isset($_POST['prog'])){
	$query = "SELECT *, IF (A.exam_taken = 0,'No', 'Yes') AS userstatus FROM APPLICANT AS A INNER JOIN ACADYR AS B ON A.ACADYR_FK = B.ACADYR INNER JOIN ACC AS C ON A.ACC_FK = C.ACCID ORDER BY A.APP_ID DESC";
		}
else{
	$query ="SELECT *, IF (A.exam_taken = 0,'No', 'Yes') AS userstatus FROM APPLICANT AS A INNER JOIN ACADYR AS B ON A.ACADYR_FK = B.ACADYR INNER JOIN ACC AS C ON A.ACC_FK = C.ACCID INNER JOIN PROGRAM AS D ON A.APP_FIRST_FK = D.PROG_ID  where D.PROG_ID = '$prog' ORDER BY A.APP_ID DESC";
}
	$query2="SELECT PROG_NAME AS PROGRAMNAME FROM PROGRAM WHERE PROG_ID = '$prog'";
