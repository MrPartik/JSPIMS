...
{   
    $bno = $_GET['batch_no'];
    $output = '';
    $results = mysqli_query($connect, "SELECT * FROM t_spare_requisition_issuance  WHERE REF_BATCH_NO = $bno");
    ...
    while($row = mysqli_fetch_assoc($results))
        {
            ...
            $output .= '<th width="10%">Request Date</th>';
            $output .= '<th width="30%">Unit</th>';
                        $results1 = mysqli_query($connect, "SELECT * from t_spare_requisition_issuance A 
                INNER JOIN t_spare_stocks as B 
                ON A.REF_STOCK_ID = B.STOCK_ID
                WHERE A.REF_BATCH_NO = $row[REF_BATCH_NO]");
            ...
            $output .= '<td class="item_unit" id="item_unit">'.$row1['DATEREQUEST'].'</td></tr>';
            } 
            $output .= '</table></div>';
            $output .= '</div>';   
        }                     
            ...
            else
            {
                ...
                function(isConfirm){
                    if (isConfirm) {
                        $.ajax({
                                    type: 'POST',
                                    url: 'INCLUDES/O_update_request_changestatus_issue.php',
                                    async: false,
                                    data: {
                                        remarks:remarks,
                                        bno:bno
                                    ...
                                    success: function(data) {
                                        ...
                                        setTimeout(function() 
                                        {
                                            ...
                                            window.open('INCLUDES/pdf_sample_issue.php?bno='+bno, '_blank');
                                            return true;
                                        },3000);
                                    },
                                    error: function(data) {
